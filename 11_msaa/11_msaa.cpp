...
int main(int /*argc*/, char **/*argv*/) {
    ...
    CreateSimpleRenderPass(device, surfaceInfo.format, depthFormat, VK_SAMPLE_COUNT_1_BIT, 0, &renderPass); // TODO: check result
    ...
    // For coloring:
    Texture resolvedOutput = Texture::Create2D(phyDevice, device, surfaceInfo.format, {windowWidth, windowHeight},
    ...
    BufferInfo lightInfo = BufferInfo::Create(phyDevice, device, sizeof(cubeVertices), VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT);
    ...
    VkPipelineLayout trianglePipelineLayout = CreateEmptyPipelineLayout(device, sizeof(MVP) * 3 + sizeof(float) * 4 * 2, descriptors.Layout());

    VkShaderModule shaderVertex     = CreateShaderModule(device, SPV_lightning_simple_vert, sizeof(SPV_lightning_simple_vert));
    VkShaderModule shaderFragment   = CreateShaderModule(device, SPV_lightning_no_frag, sizeof(SPV_lightning_no_frag));

    VkPipeline cubePipeline = CreateSimpleVec3Pipeline(device, surfaceExtent, colorRenderPass, trianglePipelineLayout, shaderVertex, shaderFragment, true, true,
                                                       msaaSamples);
    ...
    VkDescriptorSet depthShowDS = ImGui_ImplVulkan_AddTexture(shadowMap.Depth().sampler(), shadowMap.Depth().view(), VK_IMAGE_LAYOUT_DEPTH_STENCIL_READ_ONLY_OPTIMAL);
    ...
    gridSet.SetImage(1, shadowMap.Depth().view(),
                        shadowMap.Depth().sampler(),
                        VK_IMAGE_LAYOUT_DEPTH_STENCIL_READ_ONLY_OPTIMAL);
    gridSet.SetBuffer(2, lightInfo.buffer);
    gridSet.Update(device);

    Texture colorOutput = Texture::Create2D(phyDevice, device, surfaceInfo.format, {windowWidth, windowHeight},
                                            VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_SAMPLED_BIT,
                                            msaaSamples);
    ...
    std::vector<VkFramebuffer> colorFramebuffers = CreateSimpleFramebuffers(device, colorRenderPass, windowWidth, windowHeight,
                                                                            {colorOutput.view()}, colorDepth.view(), resolvedOutput.view());
    ...
    while (!glfwWindowShouldClose(window)) {
...

        {
            ...
            static VkPipeline shadow_pipelines[] = { cubePipeline, lightPass.SimplePipeline(), lightPass.ShadowMapPipeline() };
...

        {
            ...
            vkCmdPushConstants(cmdBuffer, trianglePipelineLayout, pushFlags, 1 * sizeof(MVP), sizeof(MVP), &directionalLight.view);
            vkCmdPushConstants(cmdBuffer, trianglePipelineLayout, pushFlags, 2 * sizeof(MVP), sizeof(MVP), &directionalLight.projection);

            {
                // draw the grid
                vkCmdPushConstants(cmdBuffer, trianglePipelineLayout, pushFlags, 0 * sizeof(MVP), sizeof(MVP), &grid.transform);
            ...
            {   // our main cube
                // Cube bind and draw
                vkCmdPushConstants(cmdBuffer, trianglePipelineLayout, pushFlags, 0 * sizeof(MVP), sizeof(MVP), &cubeTransform);
            ...
            vkCmdPushConstants(cmdBuffer, trianglePipelineLayout, pushFlags, 3 * sizeof(MVP) + sizeof(glm::vec4), sizeof(directionalLight.position), &directionalLight.position);
            ...
            {   // our main cube
                ...
                vkCmdPushConstants(cmdBuffer, trianglePipelineLayout, pushFlags, 0 * sizeof(MVP), sizeof(MVP), &cubeTransform);
                vkCmdPushConstants(cmdBuffer, trianglePipelineLayout, pushFlags, 1 * sizeof(MVP), sizeof(MVP), &camera.view);
                vkCmdPushConstants(cmdBuffer, trianglePipelineLayout, pushFlags, 2 * sizeof(MVP), sizeof(MVP), &camera.projection);
                vkCmdPushConstants(cmdBuffer, trianglePipelineLayout, pushFlags, 3 * sizeof(MVP), sizeof(camera.position), &camera.position);
            ...
            {
                // draw the grid
                vkCmdBindPipeline(cmdBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, lightPipeline); //lightPass.ShadowMapPipeline());
                vkCmdPushConstants(cmdBuffer, trianglePipelineLayout, pushFlags, 0 * sizeof(MVP), sizeof(MVP), &grid.transform);
            ...
            vkCmdPipelineBarrier(cmdBuffer, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT, 0, 0, NULL, 0, NULL, 2, imageMemoryBarrier);
