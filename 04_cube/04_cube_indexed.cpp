...
BufferInfo CreateBuffer(const VkPhysicalDevice phyDevice, const VkDevice device, VkDeviceSize size, VkBufferUsageFlags usageFlags) {
...
int main(int /*argc*/, char **/*argv*/) {
    ...
    std::vector<float> cubeReducedVertices;
    std::vector<uint32_t> cubeVertexIndices;

    const size_t stride = 5;

    // brain-dead way to reduce common vertices
    // Iterate over the number of vertices
    // note: at the moment each vertex data is not stored in a separate container, just a set of floats
    for (size_t idx = 0; idx < sizeof(cubeVertices) / sizeof(float); idx += stride) {
        bool vertexFound = false;

        // Check each vertex if it is already in our reduced list
        for (size_t vidx = 0; vidx < cubeReducedVertices.size(); vidx += stride) {
            const float *currentSavedPtr = cubeReducedVertices.data() + vidx;

            if (memcmp(&cubeVertices[idx], currentSavedPtr, sizeof(float) * stride) == 0) {
                vertexFound = true;
                // save the current vertex index to the vertex buffer list
                cubeVertexIndices.push_back(vidx / stride);
                break;
            }
        }

        // vertex does not exist currently, add a new entry
        if (!vertexFound) {
            size_t vertexIdx = cubeReducedVertices.size() / stride;

            cubeReducedVertices.insert(
                cubeReducedVertices.end(),
                &cubeVertices[idx],
                &cubeVertices[idx + stride]);

            cubeVertexIndices.push_back(vertexIdx);
        }
    }

    printf("-> Cube input vertex size: %lu (count: %lu)\n", sizeof(cubeVertices), sizeof(cubeVertices) / sizeof(cubeVertices[0]));
    printf("-> Cube reduced vertex size: %lu (count: %lu)\n", cubeReducedVertices.size() * sizeof(float), cubeReducedVertices.size());
    printf("-> Index buffer size: %lu (count: %lu)\n", cubeVertexIndices.size() * sizeof(uint32_t), cubeVertexIndices.size());

    BufferInfo cubeIndexingInfo = CreateBuffer(phyDevice, device, cubeVertexIndices.size() * sizeof(uint32_t), VK_BUFFER_USAGE_INDEX_BUFFER_BIT);
    {
        void* ptr = nullptr;
        vkMapMemory(device, cubeIndexingInfo.memory, 0, VK_WHOLE_SIZE, 0, &ptr);
        memcpy(ptr, cubeVertexIndices.data(), cubeVertexIndices.size() * sizeof(uint32_t));
        vkUnmapMemory(device, cubeIndexingInfo.memory);
    }

    BufferInfo cubeVertexInfo = CreateBuffer(phyDevice, device, cubeReducedVertices.size() * sizeof(float), VK_BUFFER_USAGE_VERTEX_BUFFER_BIT);

    // Upload vertex
    void* GPUPtr = nullptr;
    vkMapMemory(device, cubeVertexInfo.memory, 0, VK_WHOLE_SIZE, 0, &GPUPtr);
    // TODO: check return value

    memcpy(GPUPtr, cubeReducedVertices.data(), cubeReducedVertices.size() * sizeof(float));

    vkUnmapMemory(device, cubeVertexInfo.memory);
    ...
    while (!glfwWindowShouldClose(window)) {
...

        {
            ...
            ImGui::Checkbox("Use depth testingn", &useDepth);

            static bool rotationAutoInc = true;
...

        {
...

            {
                ...
                vkCmdBindIndexBuffer(cmdBuffer, cubeIndexingInfo.buffer, 0, VK_INDEX_TYPE_UINT32);
    ...
    vkDestroyBuffer(device, cubeVertexInfo.buffer, nullptr);
    vkFreeMemory(device, cubeVertexInfo.memory, nullptr);

    vkDestroyBuffer(device, cubeIndexingInfo.buffer, nullptr);
    vkFreeMemory(device, cubeIndexingInfo.memory, nullptr);
