...
int main(int /*argc*/, char **/*argv*/) {
    ...
    VkPipeline cubePipeline = CreateSimpleVec3Pipeline(device, surfaceExtent, renderPass, trianglePipelineLayout, shaderVertex, shaderFragment, true, true);
    ...
    lightPass.BuildPipeline(device, surfaceExtent, renderPass, trianglePipelineLayout);
    ...
    CreateSimpleRenderPass(device, surfaceInfo.format, depthFormat, &colorRenderPass);

    Texture colorOutput = Texture::Create2D(phyDevice, device, surfaceInfo.format, {windowWidth, windowHeight},
                                            VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_SAMPLED_BIT);

    std::vector<VkFramebuffer> colorFramebuffers = CreateSimpleFramebuffers(device, colorRenderPass, windowWidth, windowHeight, {colorOutput.view()}, depthView);

    // Post Process pass
    PostProcessPass postProcessPass;
    postProcessPass.BuildPipeline(device, {windowWidth, windowHeight}, renderPass);

    postProcessPass.BindInputImage(device, colorOutput);
    ...
    while (!glfwWindowShouldClose(window)) {
...

        {
            ...
            VkImageMemoryBarrier imageMemoryBarrier;
            {
                ...
                imageMemoryBarrier.newLayout = VK_IMAGE_LAYOUT_GENERAL; //VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
                imageMemoryBarrier.srcQueueFamilyIndex = 0;
                imageMemoryBarrier.dstQueueFamilyIndex = 0;
                imageMemoryBarrier.image = colorOutput.image();
